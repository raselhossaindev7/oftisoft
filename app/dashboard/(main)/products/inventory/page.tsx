    ...
    return (
        ...
        <div className="space-y-8">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Monitor stock levels, download health, and digital asset versioning.</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    ...
                    <Button 
                        className="gap-2 rounded-xl h-9 shadow-lg shadow-primary/20"
                        onClick={handleSync}
                        disabled={isSyncing}
                    >
            ...
            <div className="grid md:grid-cols-3 gap-4">
                <Card className="border-border/50">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Digital Products</CardTitle>
                ...
                <Card className="border-border/50">
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Physical</CardTitle>
                        <FileCode className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-3xl font-bold">{physicalProducts}</div>
                ...
                <Card className={`border-border/50 ${stockWarnings > 0 ? "border-orange-500/20 bg-orange-500/5" : ""}`}>
                    <CardHeader className="flex flex-row items-center justify-between pb-2">
                        <CardTitle className="text-sm font-medium text-orange-500">Stock Alerts</CardTitle>
                        <AlertTriangle className="h-4 w-4 text-orange-500" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-3xl font-bold">{stockWarnings}</div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="bg-muted/10 hover:bg-transparent">
                                    <TableHead className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Product Asset</TableHead>
                                    <TableHead className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Type</TableHead>
                                    <TableHead className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Current Version</TableHead>
                                    <TableHead className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Downloads</TableHead>
                                    <TableHead className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Stock/Status</TableHead>
                                    ...
                                    <TableRow key={p.id} className="hover:bg-primary/5">
                                        ...
                                        <TableCell className="text-right">
                                            <Button 
                                                variant="ghost" 
                                                size="sm" 
                                                className="h-8 rounded-lg gap-2 text-primary font-medium"
                                                onClick={() => {
            ...
            <Dialog open={isUpdateLogOpen} onOpenChange={setIsUpdateLogOpen}>
                <DialogContent className="sm:max-w-lg rounded-xl border-border/50 p-6">
                    <DialogHeader>
                        <DialogTitle className="text-lg font-semibold flex items-center gap-2">
                            <History className="w-5 h-5 text-primary" /> Update Log
                        </DialogTitle>
                        <DialogDescription className="text-sm text-muted-foreground">Recent asset and inventory changes.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-3 max-h-[300px] overflow-y-auto">
                        {updateLog.map((entry, i) => (
                            <div key={i} className="flex gap-3 p-3 rounded-lg bg-muted/20 border border-border/50">
            ...
            <Dialog open={isVersionDialogOpen} onOpenChange={(open) => { setIsVersionDialogOpen(open); if (!open) setVersionNotes(""); }}>
                <DialogContent className="sm:max-w-lg rounded-xl border-border/50 p-6">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-sm text-muted-foreground">Create a new version for {selectedProduct}.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            <Label className="text-xs font-medium text-muted-foreground">Version Notes</Label>
                            <Textarea 
                    ...
                    <DialogFooter>
                        <Button variant="ghost" className="rounded-lg h-9" onClick={() => setIsVersionDialogOpen(false)}>Cancel</Button>
                        <Button className="rounded-lg h-9 gap-2" onClick={handleSaveVersion} disabled={!versionNotes.trim()}>
            ...
            <Dialog open={isBackupOpen} onOpenChange={setIsBackupOpen}>
                <DialogContent className="sm:max-w-lg rounded-xl border-border/50 p-6">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-sm text-muted-foreground">Configure automated backup schedule and retention.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                    ...
                    <DialogFooter>
                        <Button variant="ghost" className="rounded-lg h-9" onClick={() => setIsBackupOpen(false)}>Cancel</Button>
                        <Button className="rounded-lg h-9 gap-2" onClick={handleSaveBackup} disabled={isSavingBackup}>
                            <Download className="w-4 h-4" />{isSavingBackup ? "Saving..." : "Save"}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
